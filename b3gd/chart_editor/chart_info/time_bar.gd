...
func _process(_delta: float) -> void:
	...
	$CurrentTimeLabel.text = format_time(time, has_minutes, has_hours)
	$TotalTimeLabel.text = "/ " + format_time(total, has_minutes, has_hours)

func format_time(seconds: float, has_minutes: bool, has_hours: bool = false) -> String:
	var result = ""
	...
	if has_minutes:
		if has_hours:
			result += str(int(seconds_left / 3600.0))
			seconds_left -= floor(seconds_left / 3600.0) * 3600
			result += ":"
		var minutes = str(int(seconds_left / 60.0))
		result += minutes.pad_zeros(2) if has_hours else minutes
